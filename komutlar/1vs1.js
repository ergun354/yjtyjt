...
exports.run = (client, message, args) => {
    if (!message.guild) {
      return message.channel.send(new Discord.RichEmbed().setColor('GREEN').setTitle('Eval;').setDescription(message.author.username + ', bu komutu direkt mesajda kullanamazsın.').setFooter('DBE', client.user.avatarURL).setTimestamp()); }
    let user = message.mentions.users.first();
    if (message.mentions.users.size < 2) return message.channel.send(new Discord.RichEmbed().setColor('GREEN').setTitle('5v5;').setDescription(message.author.tag + ', kullanım: !1vs1 <@kullanıcı> <@kullanıcı> .').setFooter('DBE', client.user.avatarURL).setTimestamp());
